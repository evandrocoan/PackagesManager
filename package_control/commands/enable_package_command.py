...
class EnablePackageCommand(sublime_plugin.WindowCommand, PackageDisabler):
    ...
    def run(self):
        ...
        self.ignored_package_list = load_list_setting(self.settings, 'ignored_packages')

        threading.Thread(target=self.threaded).start()

    def threaded(self):
        existing_packages = ExistingPackagesCommand()
        all_package_set = { item[0] : item for item in existing_packages.make_package_list('Enable') }

        self.package_list = [ ["", "", ""] ]
        self.package_list.extend(
            all_package_set[package] if package in all_package_set else [package, "", ""]
                for package in self.ignored_package_list )
    ...
    def on_done(self, picked_index):
        ...
        if picked_index == 0:
            ...
            if self.get_total_items_selected() < 1:
                ...
                self.reenable_package(packages, 'enable')
